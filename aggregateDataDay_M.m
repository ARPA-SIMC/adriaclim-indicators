function [day_month]=aggregateDataDay_M(data,datDate,grpBy,varargin)
...
%
%   OPTIONS
%        'aggFun': Aggragating funtion, by default 'nanmean'. Alternatives are
...
switch grpBy
    case 'S'
        day_season=NaN(size(data,1), 4);
        for iseas=1:4
            if (iseas==1)%DJF
                count=1;
                for is1=5:4:(size(I1,1)-1)                      
                    ind=intersect(find(J1==I1(is1)),b);
                    day_season(count:count+length(ind)-1,iseas)=ind;
                    %count=length(ind)+1;
                ...
                count=1;
                for is2=iseas:4:(size(I1,1)-1)
                    ind=intersect(find(J1==I1(is2)),b);
                    day_season(count:count+length(ind)-1,iseas)=ind;
                    %count=length(ind)+1;
                    count=count+length(ind);
                    clear ind
    ...
    case 'M'
        day_month=NaN(size(data,1), 12);
        for imonth=1:12
            count=1;
            for im=imonth:12:size(I1,1)             
                ind=intersect(find(J1==I1(im)),b);
                day_month(count:count+length(ind)-1,imonth)=ind;
                %count=length(ind)+1;
                count=count+length(ind);
                clear ind
            end         
